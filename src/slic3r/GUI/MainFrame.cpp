...
#ifdef __APPLE__
class ElegooSlicerTaskBarIcon : public wxTaskBarIcon
{
public:
    ElegooSlicerTaskBarIcon(wxTaskBarIconType iconType = wxTBI_DEFAULT_TYPE) : wxTaskBarIcon(iconType) {}
...
#else // _WIN32
    return wxIcon(Slic3r::var("ElegooSlicer_128px.png"), wxBITMAP_TYPE_PNG);
#endif // _WIN32
}
...
*/
    ...
    case GUI_App::EAppMode::Editor:
        ...
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("elegoo_logo.png"), wxBITMAP_TYPE_PNG), "ElegooSlicer");
        break;
...
#endif
        {
            // Elegoo: Use GUI_App::open_preferences instead of direct call so windows associations are updated on exit
...
void MainFrame::show_device(bool bBBLPrinter) 
{
    ...
    if (false && bBBLPrinter) {
        ...
        if (m_tabpanel->FindPage(m_calibration) == wxNOT_FOUND) {
            ...
            m_tabpanel->AddPage(m_calibration, _L("Calibration"), 
                            std::string("tab_calibration_active"), std::string("tab_calibration_active"), false);
        }
    ...
    else if (supportDeviceList) {
        // Case 2: Elegoo devices - PrinterManagerView only
        if (m_tabpanel->FindPage(m_printer_manager_view) != wxNOT_FOUND && 
            m_tabpanel->FindPage(m_calibration) == wxNOT_FOUND)
            return;
        ...
        // Case 3: Other devices - PrinterWebView only
        ...
        // Add PrinterWebView if needed
        if (!m_printer_view) {
            m_printer_view = new PrinterWebView(m_tabpanel);
            Bind(EVT_LOAD_PRINTER_URL, [this](LoadPrinterViewEvent& evt) {
                m_printer_view->load_url(evt.GetString(), evt.GetAPIkey());
            });
        }
...
void MainFrame::on_sys_color_changed()
{
 ...
 //   m_statusbar->update_dark_ui();
    ...
    m_diff_dialog.on_sys_color_changed();
...
void MainFrame::init_menubar_as_editor()
{
    ...
    //auto preference_item = new wxMenuItem(parent_menu, ElegooSlicerMenuPreferences + bambu_studio_id_base, _L("Preferences") + "\tCtrl+,", "");
#else
#endif
...
#ifdef __APPLE__
    wxString about_title = wxString::Format(_L("&About %s"), SLIC3R_APP_FULL_NAME);
    //auto about_item = new wxMenuItem(parent_menu, ElegooSlicerMenuAbout + bambu_studio_id_base, about_title, "");
        //parent_menu->Bind(wxEVT_MENU, [this, bambu_studio_id_base](wxEvent& event) {
        //    switch (event.GetId() - bambu_studio_id_base) {
        //        case ElegooSlicerMenuAbout:
        //            Slic3r::GUI::about();
        //            break;
        //        case ElegooSlicerMenuPreferences:
        //            CallAfter([this] {
        //                PreferencesDialog dlg(this);
        //                dlg.ShowModal();
        //#if ENABLE_GCODE_LINES_ID_IN_H_SLIDER
...
void MainFrame::load_config_file()
{
 ...
 //       return;
    wxFileDialog dlg(this, _L("Select profile to load:"),
        ...
        "config.json", "Config files (*.json;*.zip;*.elegoo_printer;*.elegoo_bundle;*.elegoo_filament)|*.json;*.zip;*.elegoo_printer;*.elegoo_bundle;*.elegoo_filament", wxFD_OPEN | wxFD_MULTIPLE | wxFD_FILE_MUST_EXIST);
     wxArrayString files;
    if (dlg.ShowModal() != wxID_OK)
        return;
    dlg.GetPaths(files);
...
#else
    SetIcon(wxIcon(var("ElegooSlicer_128px.png"), wxBITMAP_TYPE_PNG));
#endif // _WIN32
