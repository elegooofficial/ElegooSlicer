...
void Sidebar::priv::flush_printer_sync(bool restart)
{
    ...
    //btn_sync_printer->SetBackgroundColorNormal((*counter_sync_printer & 1) ? "#F8F8F8" :"#1677ff");
...
{
    StateColor not_synced_colour(std::pair<wxColour, int>(wxColour("#1677ff"), StateColor::Normal));
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(39 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        */
        ...
        StateColor btn_sync_bd_col(
                std::pair<wxColour, int>(wxColour("#1677ff"), StateColor::Pressed),
                std::pair<wxColour, int>(wxColour("#1677ff"), StateColor::Hovered),
    ...
    text_ctrl->Bind(wxEVT_SET_FOCUS, [this](wxFocusEvent& e) {
        ...
        p->m_search_bar->SetBorderColor(wxColour("#1677ff"));
...
bool Sidebar::reset_bed_type_combox_choices(bool is_sidebar_init)
{
    ...
    // Rename bed type labels for Elegoo CC printers (must run even when combo items
        ...
        try {
            if (type_3mf) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    model = Slic3r::Model::read_from_archive(path.string(), &config_loaded, &config_substitutions, en_3mf_file_type, strategy, &plate_data, &project_presets,
                          ...
                          BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                      << boost::format(", plate_data.size %1%, project_preset.size %2%, is_bbs_or_orca_3mf %3%, file_version %4% \n") % plate_data.size() %
                                          project_presets.size() % (en_3mf_file_type == En3mfType::From_BBS || en_3mf_file_type == En3mfType::From_Orca || en_3mf_file_type == En3mfType::From_Elegoo) % file_version.to_string();

                    if (en_3mf_file_type == En3mfType::From_Elegoo ||
                        ((file_version < Semver("1.3.0") && file_version > Semver("0.0.0")) &&
                        (en_3mf_file_type == En3mfType::From_Orca || en_3mf_file_type == En3mfType::From_BBS))) {
                        en_3mf_file_type = En3mfType::From_Elegoo;
                    }
                    ...
                    if(load_config && en_3mf_file_type != En3mfType::From_Elegoo) {
                    ...
                    const wxString msg_unsupported_geometry    = _L("The 3mf is not supported by ElegooSlicer, load geometry data only.");
                    const wxString msg_old_orca_geometry       = _L("The 3MF file was generated by an old ElegooSlicer version, loading geometry data only.");
                    ...
                    if (en_3mf_file_type == En3mfType::From_Elegoo) {
                    ...
                    else if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        Semver app_version = *(Semver::parse(ORCA_COMPATIBLE_VERSION));
                        // OrcaSlicer file (has OrcaSlicer tag) - compare file_version with ORCA_COMPATIBLE_VERSION
                        ...
                        if (load_config && (file_version < app_version) && file_version == Semver("2.3.1-alpha")) {
                            ...
                            if (config_substitutions.unrecogized_keys.size() > 0) {
                                ...
                                //ELEGOO: change the message to be more user friendly
                                wxString text  = wxString::Format(_L("The 3MF file was created by OrcaSlicer version %s, which is newer than the compatible version %s. Found unrecognized settings:"),
                                                                 file_version.to_string_sf(), app_version.to_string_sf());
                                text += "\n";
                                wxString context = text;
                                wxString append = _L("You'd better upgrade your software.\n");
                                context += "\n\n";
                                ...
                                if (file_version.min() != app_version.min()) {
                                    ...
                                    //                  file_version.to_string_sf(), std::string(SLIC3R_APP_FULL_NAME), app_version.to_string_sf());
                                    //ELEGOO: change the message to be more user friendly
                                    wxString text  = wxString::Format(_L("The 3MF file was created by OrcaSlicer version %s, which is newer than the compatible version %s. Some settings may not be fully compatible."),
                                                                     file_version.to_string_sf(), app_version.to_string_sf());
                                    text += "\n";
                                    log_and_show_3mf_info(text, newer_3mf_title);
                                }
                        ...
                        if (file_version <= orca_tag_start_version) {
                            ...
                            // Report that a BambuStudio project is being imported and compare with BAMBU_COMPATIBLE_VERSION
                            Semver slic3r_version = *(Semver::parse(BAMBU_COMPATIBLE_VERSION));// Temporarily use BAMBU_COMPATIBLE_VERSION version to avoid compatibility issues when loading models.
                    ...
                    if (plate_data.size() > 0) {
                        ...
                        if (load_config) {
                            ...
                            auto filament_colours = config_loaded.option<ConfigOptionStrings>("filament_colour");
                            project_filament_count = filament_colours ? filament_colours->size() : 0;
...
{
    return;// Disabled for BBL device, network plugin is not needed
...
void Plater::priv::update_title_dirty_status()
{
    ...
    wxGetApp().mainframe->update_title_colour_after_set_title();
#else
    wxGetApp().mainframe->SetTitle(title + " - " + SLIC3R_APP_NAME);
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "ElegooSlicer window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to ElegooSlicer failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
